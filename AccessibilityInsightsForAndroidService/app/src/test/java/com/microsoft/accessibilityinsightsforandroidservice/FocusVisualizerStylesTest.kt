...
class FocusVisualizerStylesTest {
    lateinit var testSubject: FocusVisualizerStyles

    lateinit var paintConstructionMock: MockedConstruction<Paint>
    lateinit var colorStaticMock: MockedStatic<Color>

    @Before
    @Throws(Exception::class)
    fun prepare() {
        paintConstructionMock = Mockito.mockConstruction(Paint::class.java)
    ...
    fun getCurrentLinePaintsReturnsAllRelevantPaints() {
        ...
        Assert.assertNotNull(paints["foregroundLine"])
        Assert.assertNotNull(paints["backgroundLine"])
    }
}
