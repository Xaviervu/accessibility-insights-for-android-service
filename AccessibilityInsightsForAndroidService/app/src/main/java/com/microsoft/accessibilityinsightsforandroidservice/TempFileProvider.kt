...
class TempFileProvider @JvmOverloads constructor(
    context: Context,
    private val workManager: WorkManager = WorkManagerHolder.getWorkManager(
        context
    )
) {
    ...
    private fun scheduleCleanOldFiles(tempDir: String?) {
        ...
        val cleanFilesWorker =
            OneTimeWorkRequest.Builder(CleanWorker::class.java)
    ...
    class CleanWorker(context: Context, workerParams: WorkerParameters) :
